...
class A2AMessageEnvelope:
    ...
    def parse(cls, message_text: str) -> Optional['A2AMessageEnvelope']:
        ...
        try:
            ...
            # First, try to parse simple format: "FROM: agent1 TO: agent2 CONTENT: message"
            if message_text.strip().startswith("FROM:") and "TO:" in message_text and "CONTENT:" in message_text:
                return cls.parse_simple_format(message_text)
    ...
    @classmethod
    def parse_simple_format(cls, message_text: str) -> Optional['A2AMessageEnvelope']:
        """Parse simple format: FROM: agent1 TO: agent2 CONTENT: message"""
        try:
            print(f"[A2A_ENVELOPE] parse_simple_format() called with: '{message_text}'")
            
            # Extract FROM agent
            from_match = message_text.find("FROM:")
            to_match = message_text.find("TO:")
            content_match = message_text.find("CONTENT:")
            
            if from_match == -1 or to_match == -1 or content_match == -1:
                print(f"[A2A_ENVELOPE] parse_simple_format() Missing required fields")
                return None
            
            # Extract FROM agent
            from_start = from_match + 5  # len("FROM:")
            from_end = to_match
            from_agent = message_text[from_start:from_end].strip()
            
            # Extract TO agent  
            to_start = to_match + 3  # len("TO:")
            to_end = content_match
            to_agent = message_text[to_start:to_end].strip()
            
            # Extract CONTENT
            content_start = content_match + 8  # len("CONTENT:")
            content = message_text[content_start:].strip()
            
            print(f"[A2A_ENVELOPE] parse_simple_format() Extracted:")
            ...
            if from_agent and to_agent and content:
                ...
                print(f"[A2A_ENVELOPE] parse_simple_format() Empty required fields")
                return None
                
        except Exception as e:
            print(f"[A2A_ENVELOPE] parse_simple_format() EXCEPTION: {e}")
            import traceback
            traceback.print_exc()
            return None
...
class A2AMessageHandler:
    ...
    def __init__(self, agent_id: str = None, registry: LocalRegistry = None):
    ...
    def send_to_agent(self, target_agent_id: str, message_text: str, conversation_id: str, metadata: Dict[str, Any] = None) -> str:
        ...
        agent_url = self.registry.lookup(target_agent_id)
        if not agent_url:
